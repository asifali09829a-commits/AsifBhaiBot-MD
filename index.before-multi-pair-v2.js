  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    try {
      ...
      if (cmd.startsWith(".pair ")) {
        const number=text.trim().slice(6).replace(/\\D/g,"");
        if(!number){
          await sock.sendMessage(msg.key.remoteJid,{text:"❌ Example: .pair 923001234567"});
          return;
        }
        try {
          const { spawn } = require("child_process");
          const child=spawn(process.execPath,[__dirname+"/pair-worker.js",number],{cwd:__dirname});
          let sent=false;
          child.stdout.on("data",async d=>{
            const out=d.toString().trim();
            if(out.startsWith("PAIRCODE:")&&!sent){
              sent=true;
              await sock.sendMessage(msg.key.remoteJid,{
                text:"🔐 *PAIRING CODE*\n\n"+out.slice(9).trim()+"\n\n📱 Dost apne WhatsApp mein: Linked Devices → Link with phone number"
              });
            }
          });
          child.stderr.on("data",d=>console.log("PAIR:",d.toString().trim()));
          await sock.sendMessage(msg.key.remoteJid,{text:"⏳ Pairing code generate ho raha hai..."});
        } catch(e) {
          await sock.sendMessage(msg.key.remoteJid,{text:"❌ Pairing failed: "+e.message});
        }
        return;
      }
