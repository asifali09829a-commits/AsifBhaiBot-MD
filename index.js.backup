...
if (cmd === ".open" || cmd === ".close") {
  if(!msg.key.remoteJid.endsWith("@g.us")){
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ Group mein use karo."});
    return;
  }
  ...
  try {
    await sock.groupSettingUpdate(
      msg.key.remoteJid,
      cmd === ".close" ? "announcement" : "not_announcement"
    );
    await sock.sendMessage(msg.key.remoteJid,{
      text: cmd === ".close"
        ? "🔒 Group closed — sirf admins message kar sakte hain."
        : "🔓 Group opened — sab members message kar sakte hain."
    });
  } catch(e) {
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ Failed. Bot ko admin permission chahiye."});
  }
  return;
}
...
if (cmd.startsWith(".subject ")) {
  ...
  const subject=cmd.slice(9).trim();
  if(!subject){
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ Example: .subject New Group Name"});
    return;
  }

  try {
    await sock.groupUpdateSubject(msg.key.remoteJid,subject);
    await sock.sendMessage(msg.key.remoteJid,{text:"✅ Group name updated."});
  } catch(e) {
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ Name change failed. Bot ko admin permission chahiye."});
  }
  return;
}

global.autoSettings=global.autoSettings||{
  welcome:true,
  goodbye:true
};

if (cmd === ".welcome on" || cmd === ".welcome off") {
  global.autoSettings.welcome=cmd.endsWith("on");
  await sock.sendMessage(msg.key.remoteJid,{
    text:"👋 Welcome system: "+(global.autoSettings.welcome?"ON ✅":"OFF ❌")
  });
  return;
}

if (cmd === ".goodbye on" || cmd === ".goodbye off") {
  global.autoSettings.goodbye=cmd.endsWith("on");
  await sock.sendMessage(msg.key.remoteJid,{
    text:"👋 Goodbye system: "+(global.autoSettings.goodbye?"ON ✅":"OFF ❌")
  });
  return;
}
...
if (cmd.startsWith(".timer ")) {
  const sec=parseInt(cmd.slice(7).trim());
  if(!Number.isFinite(sec)||sec<1||sec>86400){
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ 1-86400 seconds allowed."});
    return;
  }
  await sock.sendMessage(msg.key.remoteJid,{text:"⏱️ Timer started: "+sec+" seconds"});
  setTimeout(async()=>{
    try {
      await sock.sendMessage(msg.key.remoteJid,{text:"⏰ *Timer finished!*"});
    } catch(e){}
  },sec*1000);
  return;
}
...
if (cmd.startsWith(".countdown ")) {
  const sec=parseInt(cmd.slice(11).trim());
  if(!Number.isFinite(sec)||sec<1||sec>3600){
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ 1-3600 seconds allowed."});
    return;
  }
  await sock.sendMessage(msg.key.remoteJid,{text:"⏳ Countdown started: "+sec+" seconds"});
  setTimeout(async()=>{
    try {
      await sock.sendMessage(msg.key.remoteJid,{text:"🎉 *Countdown complete!*"});
    } catch(e){}
  },sec*1000);
  return;
}
...
if (cmd.startsWith(".remind ")) {
  ...
  const sec=parseInt(parts.shift());
  const note=parts.join(" ").trim();
  if(!Number.isFinite(sec)||sec<1||sec>86400||!note){
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ Example: .remind 60 Drink Water"});
    return;
  }
  await sock.sendMessage(msg.key.remoteJid,{text:"🔔 Reminder set for "+sec+" seconds."});
  setTimeout(async()=>{
    try {
...
if (cmd.startsWith(".calc2 ")) {
  const exp=cmd.slice(7).trim();
  if(!/^[0-9+*/%().\s-]+$/.test(exp)){
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ Sirf basic math allowed."});
    return;
  }
  try {
    const result=Function('"use strict";return ('+exp+')')();
...
global.botSettings=global.botSettings||{
  name:"AsifBhai Bot",
  prefix:"."
};

if (cmd.startsWith(".botname ")) {
  const name=cmd.slice(9).trim();
  if(!name){
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ Example: .botname AsifBhai"});
    return;
  }
  global.botSettings.name=name;
  await sock.sendMessage(msg.key.remoteJid,{text:"🤖 Bot name set: "+name});
  return;
}

if (cmd.startsWith(".setprefix ")) {
  const prefix=cmd.slice(11).trim();
  if(!prefix||prefix.length>3){
    await sock.sendMessage(msg.key.remoteJid,{text:"❌ Prefix 1-3 characters hona chahiye."});
    return;
  }
  global.botSettings.prefix=prefix;
  await sock.sendMessage(msg.key.remoteJid,{text:"⚙️ Prefix set: "+prefix});
  return;
}
...
║┃ ➳ *.bin* *.unbin*
║┃ ➳ *.hash* *.uuid* *.digits*
║┃ ➳ *.random* *.randomword* *.randomcolor*
║┃ ➳ *.trim* *.repeat* *.sort* *.unique*
║┃ ➳ *.palindrome* *.evenodd* *.prime*
║┃
║┃ 👥 *GROUP*
║┃ ➳ *.groupid* *.groupinfo*
║┃ ➳ *.members* *.admins*
...
║┃
║┃ 💬 *REPLIES*
║┃ ➳ *.love* *.angry* *.sad* *.happy*
║┃ ➳ *.cool* *.sleep* *.goodnight*
║┃ ➳ *.morning* *.welcome* *.bye* *.thanks*
║┃
║┃ 📊 *SYSTEM*
║┃ ➳ *.ping* *.alive* *.status* *.status2*
║┃ ➳ *.uptime* *.runtime*
║┃ ➳ *.device* *.memory* *.memory2*
║┃ ➳ *.cpu* *.ram* *.platform* *.node*
║┃ ➳ *.hostname* *.server* *.speed*
║┃ ➳ *.botinfo* *.owner* *.ownerinfo*
║┃ ➳ *.version* *.prefix* *.id* *.source*
║┃
║┃ ⏱️ *UTILITY*
║┃ ➳ *.timer* *.countdown* *.remind*
║┃ ➳ *.shout* *.whisper* *.length*
║┃ ➳ *.first* *.last* *.middle*
║┃
║┃ ⚙️ *SETTINGS*
║┃ ➳ *.welcome on/off*
...
if (cmd === ".botinfo")
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
    if (connection === "connecting" || qr) {
      console.log("\n📱 WhatsApp connection ready for pairing.");
    }
    ...
    if (connection === "close") {
      ...
      if (code === 515 || code === DisconnectReason.restartRequired) {
        console.log("♻️ WhatsApp requested restart. Restarting in 3 seconds...");
      } else {
